...
import { AlertTriangle, ArrowDown, ArrowLeft, ArrowUp, BookOpen, Boxes, BriefcaseBusiness, Building2, CheckCircle2, ChevronDown, CircleAlert, Clock3, Copy, Download, Eye, FileText, FolderOpen, Globe2, ImagePlus, KeyRound, LayoutDashboard, Link2, ListChecks, LogIn, MessageSquareText, PackageOpen, Pencil, Plus, Power, RefreshCw, Rocket, Search, Send, Settings2, ShieldCheck, Sparkles, Tag, Trash2, Upload, UserRound, UsersRound, X } from 'lucide-react';
...
import { articleDocumentSchema, type ArticleBlock, type ArticleDocument } from '../shared/article-document.js';
...
function App() {
  ...
  const deleteProject = async (target: any) => {
    if (busy) return notify('分发任务运行中，暂时不能删除客户项目', true);
    const confirmed = window.confirm(`确定永久删除客户项目“${target.name}”吗？\n\n该项目的公司资料、素材文件、选题、文章、分发记录和六个平台登录状态都会被删除，且无法恢复。`);
    if (!confirmed) return;
    try {
      await window.geoPublisher.deleteProject(target.id);
      await refresh();
      notify(`已永久删除客户项目：${target.name}`);
    } catch (e) {
...
function ContentCenter({ project, items, materials, topics, articles, contentCounts = {}, busy, refresh, notify, initialTab = 'overview' }: any) {
  ...
  const deleteContent = async (item: any) => {
    if (busy) return notify('分发任务运行中，暂时不能删除内容', true);
    const label = item.kind === 'material' ? '素材' : item.kind === 'topic' ? '选题' : '文章';
    if (!window.confirm(`确定删除${label}“${item.title || '未命名内容'}”吗？\n\n删除后无法恢复。已有发布记录不会被删除。`)) return;
    try { await window.geoPublisher.contentDelete(project.id, item.id); notify(`已删除${label}`); await refresh(); }
    catch (error) { notify((error as Error).message, true); }
  };
  const saveArticle = async (article: any, document: ArticleDocument) => {
    const payload = { ...(article.payload || {}), document, manualEditedAt: new Date().toISOString() };
    delete payload.quality;
    await window.geoPublisher.contentSave(project.id, { id: article.id, kind: 'article', title: document.title, payload });
  ...
  const renderItem = (item: any) => {
    ...
    const evergreen = item.kind === 'topic' && item.reusePolicy === 'evergreen';
    return <div className={item.kind === 'article' ? 'content-item article-preview-trigger' : 'content-item'} key={item.id} role={item.kind === 'article' ? 'button' : undefined} tabIndex={item.kind === 'article' ? 0 : undefined} onClick={openPreview} onKeyDown={(event) => { if (item.kind === 'article' && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); openPreview(); } }}><div className="content-item-icon">{item.kind === 'material' ? <Tag size={17}/> : <FileText size={17}/>}</div><div className="content-item-main"><strong>{item.title || '未命名内容'}</strong><div className="content-item-meta"><Badge tone={item.status === 'used' ? 'neutral' : item.status === 'approved' || item.status === 'ready' ? 'success' : item.status === 'paused' ? 'warning' : 'neutral'}>{contentStatusLabels[item.status] || '处理中'}</Badge><span>{typeLabel}</span>{item.kind === 'topic' && <span>使用 {item.usageCount || 0} 次</span>}{evergreen && <Badge tone="success">常青</Badge>}{item.platform && <span>{platformLabels[item.platform] || '目标平台'}</span>}</div>{item.kind === 'material' && payload.summary && <p>{payload.summary}</p>}{item.kind === 'topic' && payload.scenario && <p>{payload.scenario}</p>}{item.kind === 'article' && payload.topicId && <p>来源选题：{topics.find((topic: any) => topic.id === payload.topicId)?.title || '已删除选题'}</p>}</div>{item.kind === 'article' && <div className="content-item-actions"><Button variant="outline" icon={Eye} onClick={(event) => { event.stopPropagation(); setPreviewArticle(item); }}>预览</Button><Button variant="ghost" icon={Pencil} disabled={busy} onClick={(event) => { event.stopPropagation(); setEditingArticle(item); }}>修改</Button><Button variant="ghost" icon={Trash2} disabled={busy} onClick={(event) => { event.stopPropagation(); void deleteContent(item); }}>删除</Button></div>}{item.kind === 'topic' && <div className="content-item-actions"><Button variant="ghost" disabled={busy} onClick={() => void updateTopic(item, { reusePolicy: evergreen ? 'standard' : 'evergreen' }, evergreen ? '已取消常青主题，历史使用记录仍会保留' : '已设为常青主题，后续将生成新的角度变体')}>{evergreen ? '取消常青' : '继续做这个主题'}</Button><Button variant="ghost" disabled={busy} onClick={() => void updateTopic(item, { status: 'paused' }, '选题已暂停')}>暂停</Button><Button variant="ghost" icon={Trash2} disabled={busy} onClick={() => void deleteContent(item)}>删除</Button></div>}</div>;
  };
  const renderMaterial = (item: any) => { const payload = item.payload || {}; const pending = payload.mediaType === 'image' && payload.analysisStatus !== 'analyzed'; return <article className="material-card" key={item.id}><MaterialThumbnail projectId={project.id} item={item}/><div className="material-card-body"><div className="material-card-title"><strong>{item.title || '未命名图片'}</strong><Badge tone={pending ? 'warning' : 'success'}>{pending ? '待整理' : '已整理'}</Badge></div><p>{payload.description || payload.summary || '等待整理图片信息'}</p><div className="material-card-foot"><div className="content-item-meta"><span>{materialCategoryLabels[item.category] || '图片'}</span>{Array.isArray(payload.uses) && payload.uses.map((use: string) => <span key={use}>{use === 'cover' ? '可做封面' : use === 'body' ? '正文配图' : use === 'brand' ? '品牌展示' : '证明素材'}</span>)}</div><Button variant="ghost" icon={Trash2} disabled={busy} onClick={() => void deleteContent(item)}>删除</Button></div></div></article>; };
  return <div className="content-center">
    ...
    {previewArticle && <ArticlePreview projectId={project.id} article={previewArticle} topicTitle={topics.find((topic: any) => topic.id === previewArticle.payload?.topicId)?.title} onEdit={() => { setEditingArticle(previewArticle); setPreviewArticle(null); }} onClose={() => setPreviewArticle(null)} />}
    {editingArticle && <ArticleEditor project={project} materials={materials} article={editingArticle} onSave={(document) => saveArticle(editingArticle, document)} onClose={() => setEditingArticle(null)} notify={notify} />}
  </div>;
}
...
function ArticlePreview({ projectId, article, topicTitle, onEdit, onClose }: { projectId: string; article: any; topicTitle?: string; onEdit: () => void; onClose: () => void }) {
  const dialogRef = useRef<HTMLDialogElement>(null);
  ...
  const renderBlock = (block: any, index: number) => {
    ...
    if (block?.type === 'image' && (block.materialId || block.src)) return <figure key={index}>{block.materialId ? <ArticleMaterialImage projectId={projectId} materialId={block.materialId} alt={block.alt || ''}/> : <img src={block.src} alt={block.alt || ''} loading="lazy" referrerPolicy="no-referrer"/>}{(block.caption || block.alt) && <figcaption>{block.caption || block.alt}</figcaption>}</figure>;
    return null;
  };
  return <dialog ref={dialogRef} className="article-preview-sheet" aria-labelledby="article-preview-title" onCancel={(event) => { event.preventDefault(); onClose(); }}>
    <div className="article-preview-layout">
      <header className="article-preview-head"><div><p className="eyebrow">文章预览</p><h2 id="article-preview-title">{document.title || article.title || '未命名文章'}</h2><div className="article-preview-meta"><Badge tone={article.status === 'ready' ? 'success' : 'neutral'}>{statusLabel}</Badge>{payload.articleType && <span>{articleTypeLabels[payload.articleType] || '文章'}</span>}{article.platform && <span>{platformLabels[article.platform] || '目标平台'}</span>}</div></div><div className="article-preview-actions"><Button type="button" variant="outline" icon={Pencil} onClick={onEdit}>修改文章</Button><Button type="button" variant="ghost" icon={X} onClick={onClose} aria-label="关闭文章预览" /></div></header>
...
const articleBlockLabels: Record<ArticleBlock['type'], string> = { paragraph: '正文', heading: '小标题', list: '列表', quote: '引用', divider: '分隔线', image: '图片' };

function emptyArticleBlock(type: ArticleBlock['type']): ArticleBlock {
  if (type === 'heading') return { type, level: 2, text: '新小标题' };
  if (type === 'list') return { type, ordered: false, items: ['新列表项'] };
  if (type === 'quote') return { type, text: '新引用' };
  if (type === 'divider') return { type };
  if (type === 'image') return { type, materialId: '', alt: '', caption: '' } as ArticleBlock;
  return { type, text: '新段落' };
}

function ArticleEditor({ project, materials, article, onSave, onClose, notify }: { project: any; materials: any[]; article: any; onSave: (document: ArticleDocument) => Promise<void>; onClose: () => void; notify: (message: string, error?: boolean) => void }) {
  const dialogRef = useRef<HTMLDialogElement>(null);
  const source = article.payload?.document || {};
  const [title, setTitle] = useState(String(source.title || article.title || ''));
  const [summary, setSummary] = useState(String(source.summary || ''));
  const [tags, setTags] = useState(Array.isArray(source.tags) ? source.tags.join('，') : '');
  const [blocks, setBlocks] = useState<ArticleBlock[]>(Array.isArray(source.blocks) ? source.blocks : []);
  const [saving, setSaving] = useState(false);
  const [validationError, setValidationError] = useState('');
  const [imagePickerIndex, setImagePickerIndex] = useState<number | null>(null);
  const [availableMaterials, setAvailableMaterials] = useState<any[]>(materials.filter((item: any) => item.payload?.mediaType === 'image'));
  const [uploadingImage, setUploadingImage] = useState(false);

  useEffect(() => {
    let active = true;
    void window.geoPublisher.contentList(project.id, 'material').then((result) => {
      if (active) setAvailableMaterials(result.items.filter((item: any) => item.payload?.mediaType === 'image'));
    }).catch((error) => notify((error as Error).message, true));
    return () => { active = false; };
  }, [project.id]);
  useEffect(() => { const dialog = dialogRef.current; if (dialog && !dialog.open) dialog.showModal(); return () => { if (dialog?.open) dialog.close(); }; }, []);
  const replaceBlock = (index: number, block: ArticleBlock) => setBlocks((current) => current.map((item, itemIndex) => itemIndex === index ? block : item));
  ...
  const chooseArticleImage = (material: any) => {
    if (imagePickerIndex === null) return;
    const next: ArticleBlock = { type: 'image', materialId: material.id, alt: String(material.payload?.description || material.payload?.summary || material.title || ''), caption: '' };
    setBlocks((current) => imagePickerIndex >= current.length ? [...current, next] : current.map((block, index) => index === imagePickerIndex ? { ...next, ...(current[index]?.type === 'image' ? { alt: current[index].alt, caption: current[index].caption } : {}) } : block));
    setImagePickerIndex(null);
  };
  const uploadArticleImage = async () => {
    setUploadingImage(true);
    try {
      const result = await window.geoPublisher.contentChooseMaterial(project.id);
      if (result.canceled || result.items.length === 0) return;
      const images = result.items.filter((item: any) => item.payload?.mediaType === 'image');
      setAvailableMaterials((current) => [...images, ...current].filter((item, index, all) => all.findIndex((candidate) => candidate.id === item.id) === index));
      if (images.length === 1) chooseArticleImage(images[0]);
      else notify(`已上传 ${images.length} 张图片，请选择一张插入正文`);
    } catch (error) { notify((error as Error).message, true); }
    finally { setUploadingImage(false); }
  };
  const submit = async () => {
    const parsed = articleDocumentSchema.safeParse({ title, summary: summary.trim() || undefined, tags: tags.split(/[,，\n]/).map((tag) => tag.trim()).filter(Boolean), blocks });
    if (!parsed.success) { setValidationError(parsed.error.issues[0]?.message || '文章内容不完整'); return; }
    setSaving(true); setValidationError('');
    try { await onSave(parsed.data); } catch (error) { setValidationError((error as Error).message); setSaving(false); }
  };
  return <dialog id="article-editor" ref={dialogRef} className="article-editor-sheet" aria-labelledby="article-editor-title" onCancel={(event) => { event.preventDefault(); if (!saving) onClose(); }}>
    <div className="article-editor-layout">
      <header className="article-editor-head"><div><p className="eyebrow">修改文章</p><h2 id="article-editor-title">编辑标题、正文结构和关键词</h2></div><Button type="button" variant="ghost" icon={X} disabled={saving} onClick={onClose} aria-label="关闭文章编辑" /></header>
      <div className="article-editor-scroll">
        <label className="article-editor-field"><span>文章标题</span><input id="article-editor-title-input" value={title} maxLength={100} onChange={(event) => setTitle(event.target.value)} /></label>
        <label className="article-editor-field"><span>文章摘要（可选）</span><textarea value={summary} maxLength={120} rows={3} onChange={(event) => setSummary(event.target.value)} /></label>
        <label className="article-editor-field"><span>话题或关键词</span><input value={tags} placeholder="使用逗号分隔，例如：人工智能，企业管理" onChange={(event) => setTags(event.target.value)} /></label>
        <section className="article-block-editor"><div className="article-block-editor-title"><div><strong>正文内容</strong><p>每一块都可单独修改、排序或删除。</p></div><select aria-label="添加正文块" defaultValue="" onChange={(event) => { if (event.target.value === 'image') setImagePickerIndex(blocks.length); else if (event.target.value) setBlocks((current) => [...current, emptyArticleBlock(event.target.value as ArticleBlock['type'])]); event.target.value = ''; }}><option value="" disabled>添加内容块</option><option value="paragraph">正文段落</option><option value="heading">小标题</option><option value="list">列表</option><option value="quote">引用</option><option value="divider">分隔线</option><option value="image">插入图片</option></select></div>
          {blocks.map((block, index) => <div className="article-block-row" data-block-index={index} data-block-type={block.type} key={`${block.type}-${index}`}><div className="article-block-row-head"><Badge>{block.type === 'heading' ? `${block.level === 3 ? '三级' : '二级'}小标题` : articleBlockLabels[block.type]}</Badge><div><button type="button" disabled={index === 0} onClick={() => moveBlock(index, -1)} aria-label="上移"><ArrowUp size={15}/></button><button type="button" disabled={index === blocks.length - 1} onClick={() => moveBlock(index, 1)} aria-label="下移"><ArrowDown size={15}/></button><button type="button" onClick={() => setBlocks((current) => current.filter((_, itemIndex) => itemIndex !== index))} aria-label="删除内容块"><Trash2 size={15}/></button></div></div>
            {block.type === 'heading' && <><select value={block.level} onChange={(event) => replaceBlock(index, { ...block, level: Number(event.target.value) as 2 | 3 })}><option value={2}>二级小标题</option><option value={3}>三级小标题</option></select><textarea rows={2} value={block.text} onChange={(event) => replaceBlock(index, { ...block, text: event.target.value })}/></>}
            {(block.type === 'paragraph' || block.type === 'quote') && <textarea rows={block.type === 'paragraph' ? 5 : 3} value={block.text} onChange={(event) => replaceBlock(index, { ...block, text: event.target.value })}/>}
            {block.type === 'list' && <><label className="article-inline-check"><input type="checkbox" checked={block.ordered} onChange={(event) => replaceBlock(index, { ...block, ordered: event.target.checked })}/>显示序号</label><textarea rows={Math.max(3, block.items.length)} value={block.items.join('\n')} onChange={(event) => replaceBlock(index, { ...block, items: event.target.value.split('\n') })}/><small>每行是一条列表内容</small></>}
            {block.type === 'divider' && <p className="article-divider-preview">分隔线</p>}
            {block.type === 'image' && <div className="article-image-block">{block.materialId ? <ArticleMaterialImage projectId={project.id} materialId={block.materialId} alt={block.alt || ''}/> : block.src ? <img src={block.src} alt={block.alt || ''}/> : <div className="article-image-placeholder"><ImagePlus size={28}/><span>尚未选择图片</span></div>}<div className="article-image-fields"><div><Button type="button" variant="outline" icon={ImagePlus} onClick={() => setImagePickerIndex(index)}>{block.materialId || block.src ? '更换图片' : '选择图片'}</Button></div><input value={block.alt || ''} placeholder="图片替代文字（可选）" onChange={(event) => replaceBlock(index, { ...block, alt: event.target.value })}/><input value={block.caption || ''} placeholder="图片下方说明（可选）" onChange={(event) => replaceBlock(index, { ...block, caption: event.target.value })}/></div></div>}
          </div>)}
          {blocks.length === 0 && <Empty icon={FileText} title="正文不能为空" description="请添加至少一个正文段落。"/>}
        </section>
        {validationError && <p className="article-editor-error"><CircleAlert size={15}/>{validationError}</p>}
      </div>
      <footer className="article-editor-foot"><p>保存后，后续填充和发布会使用修改后的内容。</p><div><Button type="button" variant="outline" disabled={saving} onClick={onClose}>取消</Button><Button type="button" icon={CheckCircle2} disabled={saving} onClick={() => void submit()}>{saving ? '保存中' : '保存修改'}</Button></div></footer>
    </div>
    {imagePickerIndex !== null && <ArticleImagePicker projectId={project.id} materials={availableMaterials} uploading={uploadingImage} choose={chooseArticleImage} upload={uploadArticleImage} close={() => setImagePickerIndex(null)}/>}
  </dialog>;
}

function ArticleMaterialImage({ projectId, materialId, alt }: { projectId: string; materialId: string; alt: string }) {
  const [src, setSrc] = useState('');
  useEffect(() => { let active = true; void window.geoPublisher.materialThumbnail(projectId, materialId).then((value) => { if (active) setSrc(value.dataUrl); }).catch(() => undefined); return () => { active = false; }; }, [projectId, materialId]);
  return src ? <img src={src} alt={alt}/> : <div className="article-image-placeholder"><ImagePlus size={26}/><span>正在加载图片</span></div>;
}

function ArticleImagePicker({ projectId, materials, uploading, choose, upload, close }: { projectId: string; materials: any[]; uploading: boolean; choose: (material: any) => void; upload: () => void; close: () => void }) {
  return <div className="article-image-picker-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}><section className="article-image-picker" aria-label="选择正文图片"><header><div><strong>选择正文图片</strong><p>从当前客户素材库选择，或从电脑上传新图片。</p></div><Button type="button" variant="ghost" icon={X} onClick={close} aria-label="关闭图片选择"/></header><div className="article-image-picker-actions"><Button type="button" icon={Upload} disabled={uploading} onClick={() => void upload()}>{uploading ? '正在上传' : '从电脑上传'}</Button></div>{materials.length ? <div className="article-image-picker-grid">{materials.map((material) => <button type="button" key={material.id} onClick={() => choose(material)}><MaterialThumbnail projectId={projectId} item={material}/><strong>{material.title || '未命名图片'}</strong><small>{material.payload?.description || material.payload?.summary || '图片素材'}</small></button>)}</div> : <Empty icon={ImagePlus} title="素材库还没有图片" description="点击“从电脑上传”，上传后会自动保存到当前客户素材库。"/>}</section></div>;
}
