...
import { AlertTriangle, Boxes, BriefcaseBusiness, Building2, CheckCircle2, ChevronDown, ContactRound, Copy, Download, FileText, FolderOpen, Globe2, KeyRound, LayoutDashboard, Link2, ListChecks, LogIn, MessageSquareText, PackageOpen, Plus, Power, RefreshCw, Search, Send, Settings2, ShieldCheck, Sparkles, Tag, UserRound, UsersRound, X } from 'lucide-react';
...
function ContentCenter({ project, items, materials, topics, articles, busy, refresh, notify }: any) {
  ...
  const renderItem = (item: any) => {
    const payload = item.payload || {}; const typeLabel = item.kind === 'topic' ? (payload.articleType || '待分类结构') : item.kind === 'article' ? (payload.articleType || '文章') : (item.category || '待确认');
    return <div className="content-item" key={item.id}><div className="content-item-icon">{item.kind === 'material' ? <Tag size={17}/> : <FileText size={17}/>}</div><div className="content-item-main"><strong>{item.title || '未命名内容'}</strong><div className="content-item-meta"><Badge tone={item.status === 'used' ? 'neutral' : item.status === 'approved' || item.status === 'ready' ? 'success' : item.status === 'paused' ? 'warning' : 'neutral'}>{item.status === 'approved' ? '待使用' : item.status === 'used' ? '已使用' : item.status === 'pending_review' ? '待确认' : item.status}</Badge><span>{typeLabel}</span>{item.kind === 'topic' && <span>使用 {item.usageCount || 0} 次</span>}{item.platform && <span>{platformLabels[item.platform] || item.platform}</span>}</div>{item.kind === 'material' && payload.summary && <p>{payload.summary}</p>}{item.kind === 'topic' && payload.scenario && <p>{payload.scenario}</p>}{item.kind === 'article' && payload.topicId && <p>来源选题：{topics.find((topic: any) => topic.id === payload.topicId)?.title || '已归档选题'}</p>}</div>{item.kind === 'topic' && <div className="content-item-actions"><Button variant="ghost" disabled={busy} onClick={() => void updateTopic(item, { reusePolicy: 'evergreen' }, '已设为常青主题，后续将生成新的角度变体')}>{item.reusePolicy === 'evergreen' ? '常青主题' : '继续做这个主题'}</Button><Button variant="ghost" disabled={busy} onClick={() => void updateTopic(item, { status: 'paused' }, '选题已暂停')}>暂停</Button></div>}</div>;
  };
  return <div className="content-center"><div className="content-center-head"><div><p className="eyebrow">当前客户 · {project?.name}</p><h2>内容中心</h2><p className="muted">素材、选题和文章分开管理，WorkBuddy 会从这里读取和保存内容。</p></div><div className="content-head-actions">{tab === 'material' && <Button icon={Plus} onClick={importMaterial}>添加素材</Button>}<Button variant="outline" icon={RefreshCw} onClick={() => void refresh()}>刷新</Button></div></div><div className="content-tabs">{[['overview', '总览'], ['material', `素材 ${counts.material}`], ['topic', `选题 ${counts.topic}`], ['article', `文章 ${counts.article}`]].map(([id, label]) => <button key={id} className={tab === id ? 'active' : ''} onClick={() => { setTab(id as any); setStatus('all'); setCategory('all'); }}>{label}</button>)}</div><div className="content-toolbar"><div className="content-search"><Search size={16}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索标题、场景或素材摘要" /></div><select aria-label="按状态筛选" value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">全部状态</option><option value="approved">待使用</option><option value="used">已使用</option><option value="ready">待分发</option><option value="pending_review">待确认</option><option value="paused">已暂停</option></select>{categories.length > 0 && <select aria-label="按分类筛选" value={category} onChange={(event) => setCategory(event.target.value)}><option value="all">全部分类</option>{categories.map((value) => <option key={value} value={value}>{value}</option>)}</select>}</div>{tab === 'overview' && <div className="content-overview-grid"><div><strong>{topics.filter((item: any) => item.status === 'approved').length}</strong><span>待使用选题</span></div><div><strong>{topics.filter((item: any) => item.reusePolicy === 'evergreen').length}</strong><span>常青主题</span></div><div><strong>{materials.filter((item: any) => item.status === 'pending_review' || item.category === 'pending_review').length}</strong><span>待确认素材</span></div><div><strong>{articles.filter((item: any) => item.status === 'ready').length}</strong><span>待分发文章</span></div></div>}{filtered.length ? <div className="content-items">{filtered.map(renderItem)}</div> : <Empty icon={PackageOpen} title={tab === 'material' ? '还没有素材' : tab === 'topic' ? '还没有选题' : tab === 'article' ? '还没有文章' : '内容库为空'} description={tab === 'material' ? '上传资料或粘贴文字后，系统会自动归类并建立检索索引。' : '让 WorkBuddy 在当前客户项目中生成内容，保存后会自动出现在这里。'}/>}</div>;
}
