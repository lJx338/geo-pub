...
function View({ view, project, projects, status, items, articles, topics, materials, distributions, busy, setView, openPlatform, edit, switchProject, refresh, workBuddy, setWorkBuddy, update, setUpdate, connect, notify }: any) {
  ...
  if (view === 'overview') return <><div className="hero-row"><div><p className="eyebrow">当前客户项目</p><h2 id="current-project-title">{project.name}</h2><p className="muted" id="current-project-company">{project.companyName || '未填写公司全称'} · {project.industry || '未填写行业'}</p></div><Button variant="outline" icon={Send} onClick={() => setView('distribution')}>进入分发</Button></div><div className="stat-grid"><Stat icon={FileText} label="文章" value={articles.length}/><Stat icon={ListChecks} label="选题" value={topics.length}/><Stat icon={Boxes} label="素材" value={materials.length}/><Stat icon={Globe2} label="平台" value="6"/></div><Card><div className="card-head"><div><h3>平台账号</h3><p>每个客户项目分别保存平台登录状态。</p></div><Button variant="ghost" onClick={() => setView('accounts')}>查看全部</Button></div><div className="platform-grid">{platforms.map(([id,label]) => <button data-platform={id} key={id} onClick={() => openPlatform(id)} className="platform-tile"><span className="platform-dot"/><strong>{label}</strong><small>打开登录页</small></button>)}</div></Card></>;
  ...
  if (view === 'content') return <ContentCenter project={project} items={items} materials={materials} topics={topics} articles={articles} busy={busy} refresh={refresh} notify={notify}/>;
  ...
  if (view === 'guide') return <GuideView project={project} workBuddy={workBuddy} setView={setView} connect={connect} notify={notify}/>;
...
function GuideView({ project, workBuddy, setView, connect, notify }: any) {
  const steps = [
    ['1', '建立客户项目', '在“客户项目”中填写客户资料。不同客户的内容、平台账号和登录状态彼此隔离。', UsersRound, 'projects'],
    ['2', '登录平台账号', '进入“平台账号”，逐个平台完成登录。只登录需要使用的平台即可。', Globe2, 'accounts'],
    ['3', '连接 WorkBuddy', '点击左侧“连接 WorkBuddy”，把自动复制的指令粘贴给 WorkBuddy，让它使用当前客户项目。', Link2, null],
    ['4', '整理图片素材', '在“内容中心”添加产品、设备、案例或资质图片，再让 WorkBuddy 整理一次，后续生成文章会直接检索。', ImagePlus, 'content'],
    ['5', '生成和检查文章', '让 WorkBuddy 生成选题和文章。文章保存后，在“内容中心”预览标题、段落、小标题、列表和图片。', FileText, 'content'],
    ['6', '选择平台并分发', '从文章进入“分发”，选择一个或多个平台。先填充检查，确认无误后再明确执行真实发布。', Send, 'distribution'],
  ] as const;
  const features = [
  ...
  return <div className="guide-page">
    ...
    <section className="guide-section"><div className="guide-section-heading"><p className="eyebrow">首次使用</p><h3>按这 6 步开始</h3></div><div className="guide-steps">{steps.map(([number, title, description, Icon, destination]) => <div className="guide-step" key={number}><span className="guide-step-number">{number}</span><span className="guide-step-icon"><Icon size={17}/></span><div><h4>{title}</h4><p>{description}</p></div>{destination && <Button variant="ghost" onClick={() => setView(destination as View)}>前往</Button>}{title === '连接 WorkBuddy' && <Button variant="ghost" onClick={() => void connect()}>复制指令</Button>}</div>)}</div></section>
    <section className="guide-section workbuddy-tutorial"><div className="guide-section-heading"><p className="eyebrow">搭配 WorkBuddy</p><h3>从公司资料到自动发布的简易教程</h3><p>第一次按顺序完成，之后只需要查看内容和发布结果。带【】的内容需要换成自己的平台或资料。</p></div><div className="workbuddy-connect-step"><span className="guide-step-icon"><Link2 size={17}/></span><div><strong>开始前先连接 WorkBuddy</strong><p>{workBuddy.prepared ? '连接指令已经准备好；如果 WorkBuddy 无法读取当前项目，再重新连接一次。' : '点击按钮后，将自动复制的连接指令粘贴到 WorkBuddy 并发送。'}</p></div><Button icon={Link2} onClick={() => void connect()}>{workBuddy.prepared ? '重新连接' : '连接 WorkBuddy'}</Button></div><div className="workbuddy-prompt-list">{workBuddyPrompts.map((item, index) => <article className={index === workBuddyPrompts.length - 1 ? 'workbuddy-prompt automation' : 'workbuddy-prompt'} key={item.number}><header><span className="guide-step-number">{item.number}</span><div><h4>{item.title}</h4><p>{item.description}</p></div><Badge tone={index === workBuddyPrompts.length - 1 ? 'warning' : item.badge === '不会发布' ? 'success' : 'neutral'}>{item.badge}</Badge><Button variant="outline" icon={Copy} onClick={() => void copyPrompt(item.title, item.prompt)}>复制提示词</Button></header><details open={index === 0}><summary>查看提示词<span>+</span></summary><p>{item.prompt}</p></details></article>)}</div><div className="workbuddy-automation-note"><ShieldCheck size={18}/><div><strong>自动发布不要一开始就开启</strong><p>先完成公司资料、平台登录、一次仅填充和一次真实发布。确认账号与页面都正常后，再创建自动化任务。</p></div></div></section>
...
function DistributionView({ project, articles, distributions, status, busy, refresh, notify }: any) {
  ...
  const run = async () => {
    ...
    setDraft(null);
    try {
      await window.geoPublisher.runDistribution(request);
      notify(request.mode === 'publish' ? '分发任务已完成，请核对每个平台结果' : '文章填充已完成，请检查平台草稿');
      await refresh();
    } catch (error) {
...
function ContentCenter({ project, items, materials, topics, articles, busy, refresh, notify }: any) {
  const [tab, setTab] = useState<'overview' | 'material' | 'topic' | 'article'>('overview');
  ...
  const filtered = useMemo(() => items.filter((item: any) => {
    ...
    return true;
  }), [items, tab, status, category, query]);
  const counts = { material: materials.length, topic: topics.length, article: articles.length };
